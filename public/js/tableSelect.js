    ...
    */
    tableSelect.prototype.render = function (opt) {
        ...
        elem.off('click').on('click', function(e) {
            ...
            var tableBox = '<div class="tableSelect layui-anim layui-anim-upbit" style="left:'+l+';top:'+t+';border: 1px solid #d2d2d2;background-color: #fff;box-shadow: 0 2px 4px rgba(0,0,0,.12);padding:10px 10px 0 10px;position: absolute;z-Iframe:66666666;margin: 5px 0;border-radius: 2px;min-width:530px;">';
				...
				for(var i=0;i<res.data.length;i++){
            		for (var j=0;j<checkedData.length;j++) {
            			if(res.data[i][opt.checkedKey] == checkedData[j][opt.checkedKey]){
                            ...
                            var checkbox = $('#'+tableName+'').next().find('tr[data-Iframe=' + index + '] input[type="checkbox"]');
            				    checkbox.prop('checked', true).next().addClass('layui-form-checked');
                            var radio  = $('#'+tableName+'').next().find('tr[data-Iframe=' + index + '] input[type="radio"]');
